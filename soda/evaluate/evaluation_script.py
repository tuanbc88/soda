...
def evaluaterefcand(reference, candidate):
    ...
    # nervaluate <0.2 returns (results, results_per_tag); >=0.2 returns 4 values.
    _ev = evaluator.evaluate()
    results, results_per_tag = _ev[0], _ev[1]
...
def calculateSystemScore_hungarian(
    totalsemevallist,
    totalsemevallistpertag,
    newreflist,
    newcandlist
    ...
    for idx in range(len(newcandlist)):
        candidates = newcandlist[idx]
        references = newreflist[idx]
        ...
        # cost matrix (we maximize F1 → convert to cost = -F1)
        ...
        for i in range(n):
            for j in range(m):
                scores = totalsemevallist[idx][i][j]
                ...
                cost_matrix[i, j] = -f1  # maximize → minimize negative
                score_cache[(i, j)] = scores
        ...
        if n > 0 and m > 0:
            ...
            matched_scores = []
            matched_scores_per_tag = []
            total_score = 0.0

            for i, j in zip(row_ind, col_ind):
                scores = score_cache[(i, j)]
                ...
                matched_scores_per_tag.append(
                    totalsemevallistpertag[idx][i][j]
                )

            # normalize giống bản gốc
            avg_score = total_score / max(n, 1)

            selectedscores.append(avg_score)
            selectedsemevallist.extend(matched_scores)
            selectedsemevallistpertag.extend(matched_scores_per_tag)

        else:
            # edge case: only one side empty
            selectedscores.append(0.0)

    return selectedsemevallist, selectedsemevallistpertag, None, selectedscores
...
def clean_triples(triples, idx=None, log_dir=None, tag="pred"):
    cleaned = []
    dropped = []
    ...
    for t in triples:
        ...
        if not isinstance(t, (list, tuple)):
            reason = "not_list_or_tuple"

        elif len(t) != 3:
            reason = "len_not_3"

        elif not all(isinstance(x, str) for x in t):
            reason = "non_string_element"

        else:
            s, p, o = [x.strip() for x in t]

            if not s or not p or not o:
                reason = "empty_component"
            else:
                cleaned.append((s, p, o))
                continue  # valid → skip logging
    ...
    # ===== dump cleaned log =====
    if dropped and log_dir is not None and idx is not None:
        clean_log_dir = os.path.join(log_dir, "eval_logs")
        os.makedirs(clean_log_dir, exist_ok=True)

        filename = os.path.join(clean_log_dir, f"{idx}_cleaned.json")

        # nếu file đã tồn tại → append (pred + gold chung 1 file)
...
def normalize_token(x):
    ...
    # ----------------------------------
    # 3. normalize special symbols
    # ----------------------------------
    x = x.replace("&", "and")

    # ----------------------------------
    # 4. remove dot in abbreviations
    # Alan_B._Miller → Alan_B_Miller
    # ----------------------------------
    x = x.replace(".", "")

    # ----------------------------------
    # 5. normalize comma → underscore
    # ----------------------------------
    x = x.replace(",_", "_").replace(",", "_")

    # ----------------------------------
    # 6. normalize spaces → underscore
    # ----------------------------------
    x = re.sub(r"\s+", "_", x)

    # ----------------------------------
    # 7. remove invalid chars (keep a-zA-Z0-9_)
    # ----------------------------------
    ...
    # ----------------------------------
    # 9. remove leading/trailing underscore
    # ----------------------------------
    x = x.strip("_")
...
def _to_words(x):
    """Turn a normalized token into SPACE-separated words so the token-based
    (nervaluate) scorer can award *partial* overlap credit — mirroring the WebNLG
    eval script (underscore->space, camelCase->space). Without this, entities were
    one underscore-joined token => partial == exact (see COMPARISON_vs_EDC_paper.md)."""
    if not isinstance(x, str):
        x = str(x)
    x = re.sub(r"([a-z0-9])([A-Z])", r"\1 \2", x)   # camelCase (e.g. birthPlace)
    x = x.replace("_", " ")
    x = re.sub(r"\s+", " ", x).strip().lower()
    return x


def normalize_triples_minimal(triples):
    # normalize all three elements (incl. relation) then split into words so
    # strict/exact stay the same but partial gets real token-overlap credit.
    return [
        [_to_words(normalize_token(s)), _to_words(normalize_token(r)), _to_words(normalize_token(o))]
        for (s, r, o) in triples
    ]

def compute_metrics(pred_triplets, gold_triplets, idx=None, log_dir=None):

    pred_triplets = clean_triples(pred_triplets, idx, log_dir, tag="pred")
    gold_triplets = clean_triples(gold_triplets, idx, log_dir, tag="gold")

    pred_triplets = normalize_triples_minimal(pred_triplets)
    gold_triplets = normalize_triples_minimal(gold_triplets)
    ...
    # ===== normalize =====
    def normalize_sample(triples):
        return [
            " | ".join([t.lower().strip() for t in triple])
            for triple in triples
        ]

    reflist = [normalize_sample(gold_triplets)]
    candlist = [normalize_sample(pred_triplets)]

    if not reflist[0] or not candlist[0]:
        return ({"f1": 0.0}, {"f1": 0.0}, {"f1": 0.0})

    use_hungarian = len(pred_triplets) > 8 or len(gold_triplets) > 8
    ...
    try:
        ...
        # ===== chọn strategy =====
        if use_hungarian:
            selectedsemevallist, _, _, selectedscores = calculateSystemScore_hungarian(
                totalsemevallist,
                totalsemevallistpertag,
                reflist,
                candlist
            ...
            selectedsemevallist, _, _, _ = calculateSystemScore(
                totalsemevallist,
                totalsemevallistpertag,
                reflist,
                candlist
        ...
        # ===== error log =====
        if eval_log_dir is not None and idx is not None:
            with open(os.path.join(eval_log_dir, f"{idx}_error.json"), "w") as f:
                json.dump({
                    "idx": idx,
                    "error": str(e),
                    "mode": "hungarian" if use_hungarian else "default",
                    "pred": pred_triplets,
                    "gold": gold_triplets
    ...
    # ===== aggregate =====
    if not selectedsemevallist:
        return ({"f1": 0.0}, {"f1": 0.0}, {"f1": 0.0})

    # ===== normalization (QUAN TRỌNG) =====
    if use_hungarian:
        # penalize over/under generation
        n = max(len(pred_triplets), len(gold_triplets))
    else:
        # giữ nguyên SemEval
        n = len(selectedsemevallist)

    # tránh chia 0
    n = max(n, 1)

    exact_f1 = sum(x["exact"]["f1"] for x in selectedsemevallist) / n
    strict_f1 = sum(x["strict"]["f1"] for x in selectedsemevallist) / n
    partial_f1 = sum(x["partial"]["f1"] for x in selectedsemevallist) / n

    # ===== dump =====
    if eval_log_dir is not None and idx is not None:

        filename = f"{idx}_hungarian.json" if use_hungarian else f"{idx}.json"

        with open(os.path.join(eval_log_dir, filename), "w") as f:
            json.dump({
                "idx": idx,
                "mode": "hungarian" if use_hungarian else "default",
                "pred_len": len(pred_triplets),
                "gold_len": len(gold_triplets),
                "matched_pairs": len(selectedsemevallist),
                "pred": pred_triplets,
                "gold": gold_triplets,
                "scores": {
                    "exact": exact_f1,
    ...
    # ===== free memory =====
    del totalsemevallist
    del totalsemevallistpertag
    del selectedsemevallist
...
#     pred_triplets = clean_triples(pred_triplets)
#     gold_triplets = clean_triples(gold_triplets)
...
#         skip_log_path = os.path.join(log_dir, "eval_skip_log.txt")
...
#     # ===== skip early (QUAN TRỌNG) =====
#     if len(pred_triplets) > 8 or len(gold_triplets) > 8:
#         if skip_log_path is not None:
#             with open(skip_log_path, "a") as f:
#                 f.write(f"{idx}\t{len(pred_triplets)}\t{len(gold_triplets)}\n")

#         # dump luôn sample để debug
#         if eval_log_dir is not None and idx is not None:
#             with open(os.path.join(eval_log_dir, f"{idx}_SKIPPED.json"), "w") as f:
#                 json.dump({
#                     "idx": idx,
#                     "pred": pred_triplets,
#                     "gold": gold_triplets
...
#     # ===== dump normal case =====
#     if eval_log_dir is not None and idx is not None:
#         with open(os.path.join(eval_log_dir, f"{idx}.json"), "w") as f:
#             json.dump({
#                 "idx": idx,
#                 "pred": pred_triplets,
#                 "gold": gold_triplets,
